...
export const SALayout: React.FC<SALayoutProps> = ({ active, onNavigate, children, tenantCount }) => {
  ...
  const NavContent = () => (
    <nav className="flex flex-col h-full">
      ...
      {/* Footer with Logout */}
      <div className="px-4 py-3 border-t border-slate-200/80 dark:border-white/8 space-y-2 shrink-0">
        <button
          onClick={handleLogout}
          className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl border border-red-200 dark:border-red-900/50 bg-red-50 hover:bg-red-100 dark:bg-red-950/40 text-red-600 dark:text-red-400 text-xs font-bold transition shadow-xs cursor-pointer"
        >
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-slate-50 dark:bg-[#0c1117] text-slate-900 dark:text-white font-sans">
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex flex-col w-64 shrink-0 border-r border-slate-200/80 dark:border-white/8 bg-white dark:bg-slate-900/80 backdrop-blur-xl">
        <NavContent />
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        {/* Top header bar */}
        <header className="sticky top-0 z-30 flex items-center gap-3 px-5 py-3 border-b border-slate-200/80 dark:border-white/8 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl shrink-0">
          {/* Mobile menu toggle */}
          <button
          ...
          <button
            onClick={handleLogout}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-red-200 dark:border-red-900/50 bg-red-50 hover:bg-red-100 dark:bg-red-950/40 text-red-600 dark:text-red-400 text-xs font-bold transition shadow-sm cursor-pointer"
            title="Logout of Super Admin Account"
          >
            <LogOut className="h-3.5 w-3.5" />
